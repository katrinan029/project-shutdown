...
def original_sf():
    ...
    original_results = pd.read_sql(
        'SELECT * FROM original_table', connection)
...
def scatterPlot():
    ...
    location_end_count = pd.DataFrame(location_end_count).reset_index().rename(columns={'index':"year", 'Year':"year_count"})
    ...
    location_start_count = pd.DataFrame(location_start_count).reset_index().rename(columns={'index':"year", 'Year':"year_count"})

    #Merge location end and start date counts
    location_data = pd.merge(location_start_count, location_end_count, on='year')
    location_data = location_data.rename(columns={'year_count_x':"loc_start_count", 'year_count_y':"loc_end_count"})

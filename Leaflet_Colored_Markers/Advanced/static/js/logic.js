...
for (var i = 0 ; i < 75000; i++) {
  ...
  if (element.location_end_date && (!element.dba_end_date || element.dba_end_date===undefined)) {
      layerKey = "Business_Moved_Location"; //how do I add location and and not include business end date
  }
  else if (element.dba_end_date) {
    layerKey = "Business_UnRegistered"; 
...
var newMarker = L.marker([element.location.coordinates[1], element.location.coordinates[0]], {
 icon: icons[layerKey]
 });
newMarker.addTo(layers[layerKey]);
newMarker.bindPopup("<h1>" + element.dba_name + "</h1> <hr> <h3>Neighborhood: " + element.neighborhoods_analysis_boundaries + "</h3>");
}
});
